#### Submitted

- [1] Ding J., Xing Z., Wang Y., et al. DANCE 2.0: Transforming single-cell analysis from black box to transparent workflow[J]. bioRxiv, 2025: 2025.07.17.665427.  Submitted to Nature Methods
  - Preprint: `https://www.biorxiv.org/content/10.1101/2025.07.17.665427v1`  
  - Source code: `https://github.com/OmicsML/dance`  
  - Live demo (Atlas subproject): `http://omicsml.ai:81/dance/`

#### Published

- [1] Xing Z., Liang J., Yu G., et al. Explanation Rectification Framework: An Attribution Score Generation Method Based on Standard Explanations[J]. Chinese Journal of Computers, 2025, 48(4): 949-970 (in Chinese). `doi: 10.11897/SP.J.1016.2025.00949`

#### In progress

- SpatioFlow: Modeling spatiotemporal dynamics from spatial omics data — developing a framework that fuses Unbalanced Optimal Transport and Flow Matching to reconstruct continuous cellular trajectories and predict perturbation responses (core implementation complete; validation ongoing).
